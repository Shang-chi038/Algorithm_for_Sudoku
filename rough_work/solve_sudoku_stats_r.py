...
def solve_sudoku(board, stats=None):
    ...
    stats: dictionary to track attempts and backtracks
    """
    # First call: create the stats dictionary
    if stats is None:
        stats = {'attempts': 0, 'backtracks': 0}
    ...
    for row in range(9):
        for col in range(9):
            if board[row][col] == 0:
                ...
                for val in value:   
                    stats['attempts'] += 1  # Count every attempt ✓
